  ...
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        ...
        \item[\checkmark] Orga-Dete 三模块迁移：全面失败
        ...
        \item[\checkmark] \textbf{鼠肝 bbox FL 全部完成（E4-E11）}
        \item[\checkmark] \textbf{bbox 范式到天花板确认}
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{下一步：视觉原语框架}
      \begin{itemize}
        \item[$\rightarrow$] bbox → SAM2 mask → 形态学特征
        \item[$\rightarrow$] FL 聚合 primitive 分布（非权重）
        \item[$\rightarrow$] 形态学特征不受像素尺度影响
