  ...
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        ...
        \item[\checkmark] 鼠肝 v2 14 实验全部完成
        \item[\checkmark] \textbf{CTM 全量训练：test AUC=0.80}（$<$RF-DETR 0.89）
        \item[\checkmark] \textbf{CTM 代码审计：5 bug 修复}（数据泄露+同步化零值）
        \item[\checkmark] \textbf{Phase 8 小波实验：假设被推翻}
        \item[\checkmark] \textbf{Phase 9 Slot Attention：PR-AUC=0.788}（-2.0pp）
        \item[\checkmark] \textbf{视觉原语文献调研}（FORLA/TwVP/WIPES）
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{Phase 9 关键结果}
      \begin{itemize}
        \item Slot AUC=0.868，\textbf{最强替代方案}（-2.0pp）
        \item Val AUC 0.890 $\approx$ RF-DETR 0.883
        \item PR-AUC 0.788 $>$ 0.50 $\to$ object-centric 有效
        \item 480K params（CTM 的 1/10），过拟合轻
        \item DINOv2 CLS=0.29 $\to$ Slot=0.788（+49.8pp）
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0em}
  \begin{alertblock}{下一步}
    Slot + confidence 组合 $\to$ Phase 11 对比学习（InfoNCE）$\to$ Phase 10 联邦 slot 聚合 $\to$ 论文撰写 + 专利申请
  \end{alertblock}
\end{frame}
