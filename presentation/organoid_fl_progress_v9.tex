...
\begin{frame}{总结}
  \begin{columns}[T]
    ...
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{CTM + Phase 8 关键结果}
      \begin{itemize}
        \item CTM test AUC=0.80，没超过 RF-DETR 0.89
        \item 数据泄露修复：0.907$\to$0.800（-10pp 水分）
        \item 同步化零值 bug：破坏时间相关性矩阵
        \item 小波单独 PR-AUC=0.45（$<$随机）
        \item W3 morph+wavelet=0.645（互补有效）
        \item 所有方案均 $<$RF-DETR confidence 0.82
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0em}
  \begin{alertblock}{下一步}
    Phase 9 Slot Attention（FORLA 路线）$\to$ Phase 10 联邦原语聚合 $\to$ Phase 11 对比学习 $\to$ 论文撰写 + 专利申请
  \end{alertblock}
\end{frame}
