...
\subtitle{项目进展汇报 v16 — 新增 SAM2 自蒸馏三层证据链 + 自适应蒸馏策略}
\date{2026年7月}
...
% ========================================================================
\section{MPM 跨域验证（2026.07.15）}

\begin{frame}{MPM 跨域验证：为什么要做？}
  \begin{alertblock}{核心问题}
    国自然申请书定位 \textbf{MPM（恶性胸膜间皮瘤）类器官药物筛选平台}，但：
    \begin{itemize}
      \item 团队目前 \textbf{0 张 MPM 类器官图像}
      \item 全球 \textbf{0 个公开 MPM 类器官数据集}
      \item 所有论文原始图像 = "联系作者获取"
    \end{itemize}
  \end{alertblock}
  \vspace{-0.3em}
  \begin{block}{代理验证策略}
    \begin{itemize}
      \item 用已有数据（鼠肝 89\% + MultiOrg 77.8\%）验证算法可行性
      \item 跨域到 MPM 评估 \textbf{domain gap} 和 \textbf{失效程度}
      \item 三本子联动：SURF 2026（代理）$\to$ 瑞金医工交叉（20 真实）$\to$ 国自然（100 例多中心）
    \end{itemize}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{今日三个实验（2026-07-15）}
    \begin{enumerate}
      \item RF-DETR zero-shot MPM 检测
      \item ResNet-50 跨域 UMAP 可视化
      \item SAM2 zero-shot MPM 分割
    \end{enumerate}
  \end{block}
\end{frame}

\begin{frame}{MPM 图像数据收集：3 篇论文 PDF 提取}
  \begin{block}{数据来源（3 篇开放获取论文）}
    \begin{tabular}{llcc}
      \toprule
      \textbf{论文} & \textbf{DOI} & \textbf{PDO 数} & \textbf{提取图} \\
      \midrule
      Sci Rep 2025 (Liu et al.) & 10.1038/s41598-025-25087-0 & 11 & 6 \\
      BMC Cancer 2023 (Ito et al.) & 10.1186/s12885-023-10966-4 & 7 & 6 \\
      Sci Rep 2018 (Mazzocchi et al.) & 10.1038/s41598-018-21200-8 & 2 & 6 \\
      \midrule
      \textbf{合计} & — & \textbf{20} & \textbf{18 composite} \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{Panel 分割结果}
    \begin{itemize}
      \item 自动检测白色 gap + panel 分割
      \item 按模态分类：\textbf{34 bright-field + 14 H\&E/IHC + 3 IF + 7 in-vivo}
      \item 总计 58 张独立 patch，对训练有用的是 34 张 bright-field
    \end{itemize}
  \end{block}
  \vspace{-0.3em}
  \begin{alertblock}{关键限制}
    PDF 提取的是论文图表（1000-2000 px），不是原始显微镜图像（4000+ px）。
    能做 zero-shot 评估，不能做训练。真实数据仍需联系作者。
  \end{alertblock}
\end{frame}

\begin{frame}{实验 1：RF-DETR Zero-shot MPM 检测}
  \begin{columns}[T]
    \column{0.50\textwidth}
    \begin{block}{设置}
      \begin{itemize}
        \item 模型：RF-DETR Small（鼠肝 b1 训练，mAP 89\%）
        \item 测试：34 张 MPM bright-field patch
        \item 推理：CPU，threshold=0.01
        \item 无 GT $\to$ 只能评估检测量+置信度
      \end{itemize}
    \end{block}
    \begin{alertblock}{结果}
      \begin{itemize}
        \item 总检测 3121，但 \textbf{97.8\% conf $<$ 0.05}（噪声）
        \item Median conf = \textbf{0.014}（噪声级）
        \item Max conf = 0.696（仅 1 个）
        \item conf $\geq$ 0.5 的检测：\textbf{1 个}
        \item 100\% patch 在 conf $\geq$ 0.01 有检测（弱信号）
        \item 38.2\% patch 在 conf $\geq$ 0.05 有检测
      \end{itemize}
    \end{alertblock}
    \column{0.48\textwidth}
    \begin{block}{置信度分布}
    ...
    \end{block}
    \begin{alertblock}{结论}
      RF-DETR 跨域到 MPM \textbf{基本失效}。
      但 1 个 0.696 检测说明有微弱信号。
    \end{alertblock}
  \end{columns}
\end{frame}

\begin{frame}{实验 2：ResNet-50 跨域 UMAP}
  \begin{columns}[T]
    \column{0.50\textwidth}
    \begin{block}{设置}
      \begin{itemize}
        \item 特征：ResNet-50 (ImageNet, 2048 dim)
        \item 三组：30 鼠肝 + 30 MultiOrg + 34 MPM
        \item UMAP 2D 可视化
        \item 质心间余弦距离
      \end{itemize}
    \end{block}
    \begin{block}{质心距离（余弦）}
      \begin{tabular}{lc}
        ...
        \textbf{对比} & \textbf{距离} \\
        \midrule
        Liver $\leftrightarrow$ MultiOrg & \textbf{0.056} \\
        Liver $\leftrightarrow$ MPM & \textbf{0.272} (5x!) \\
        MPM $\leftrightarrow$ MultiOrg & 0.148 \\
        \bottomrule
      \end{tabular}
    \end{block}
    \column{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/mpm/cross_domain_umap.png}
    
    \scriptsize 鼠肝/MPM/MultiOrg 三域 UMAP
  \end{columns}
\end{frame}

\begin{frame}{实验 2：跨域鸿沟的定量证据}
  \begin{block}{MPM 各 patch 到鼠肝质心距离}
    \begin{itemize}
      \item Mean: 0.4395
      \item Median: 0.4393
      \item Min: 0.2840（最近 MPM patch）
      \item Max: 0.5455（最远 MPM patch）
    \end{itemize}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{Top-5 MPM patches 最接近鼠肝}
    \scriptsize
    \begin{tabular}{rl}
      \toprule
      \textbf{距离} & \textbf{patch} \\
      \midrule
      0.284 & BMC Cancer 2023 Fig 2 (7-day organoid) \\
      0.339 & Sci Rep 2025 Fig 1 r1c1 sub3 \\
      0.379 & BMC Fig 1 r2c2 (organoid establishment) \\
      0.385 & Sci Rep 2018 Fig 1 r2c3 \\
      0.390 & Sci Rep 2025 Fig 1 r1c1 sub2 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{-0.3em}
  \begin{alertblock}{关键洞察}
    RF-DETR 高 conf=0.696 的 Sci Rep 2018 r1c1 patch，在 ResNet 空间 dist=0.466，rank 24/34——\textbf{不是最接近鼠肝的}！
    
    $\to$ RF-DETR 的高 conf 不是整体视觉相似性驱动，而是\textbf{局部纹理巧合}
  \end{alertblock}
\end{frame}

\begin{frame}{实验 3：SAM2 Zero-shot MPM 分割}
  \begin{columns}[T]
    \column{0.50\textwidth}
    \begin{block}{设置}
      \begin{itemize}
        \item SAM2 hiera\_small（鼠肝训练）
        \item Box prompt: RF-DETR conf $\geq$ 0.10（11 个 box）
        \item 评估: IoU(RF-DETR box, SAM2 mask bbox)
        \item Fit ratio: box 包含在 mask bbox 的比例
      \end{itemize}
    \end{block}
    \begin{block}{结果}
      \scriptsize
      \begin{tabular}{lc}
        ...
        \textbf{指标} & \textbf{值} \\
        \midrule
        IoU mean & \textbf{0.931} \\
        IoU median & \textbf{0.952} \\
    ...
    \begin{alertblock}{关键发现}
      \begin{itemize}
        \item 11/11 IoU $>$ 0.78
        \item 6/11 IoU $>$ 0.95
        \item RF-DETR box 几乎\textbf{完全匹配} SAM2 mask bbox
      \end{itemize}
    \end{alertblock}
    \begin{block}{解读}
      RF-DETR 跨域失效 $\neq$ 检测框定位错误！
      
      \textbf{Domain gap 在分类头}（confidence），不在 box regression
    \end{block}
  \end{columns}
\end{frame}

\begin{frame}{SAM2 IoU 分布 + 跨域鸿沟定位}
  \begin{columns}[T]
    \column{0.50\textwidth}
    \begin{block}{IoU 分布（11 个检测）}
      \scriptsize
      \begin{tabular}{lcc}
        \toprule
        \textbf{区间} & \textbf{数量} & \textbf{占比} \\
        \midrule
        0.95-1.00 & 6 & 54.5\% \\
        0.90-0.95 & 3 & 27.3\% \\
        0.80-0.90 & 1 & 9.1\% \\
    ...
    \begin{block}{相关性分析}
      \begin{itemize}
        \item Corr(RF-DETR conf, IoU) = 0.376
        \item Corr(RF-DETR conf, SAM2 score) = 0.220
        \item RF-DETR conf $\neq$ box 质量
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{三大结论}
      \begin{enumerate}
        \item RF-DETR 跨域失效\textbf{在分类头}
        \item Box regression 仍然可用（IoU 0.952）
        \item SAM2 是 \textbf{domain-agnostic} 基础
      \end{enumerate}
    \end{alertblock}
    \begin{block}{启示：自蒸馏策略}
      \begin{enumerate}
        \item SAM2 zero-shot mask $\to$ 伪 GT
        \item 用伪 GT 微调 RF-DETR 分类头
        \item 不重训 box regression（已可用）
        \item 10-20 例 MPM 就够（vs 100+ 从零训练）
      \end{enumerate}
    \end{block}
  \end{columns}
\end{frame}

\begin{frame}{SAM2 检测可视化示例}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \centering
    \textbf{Sci Rep 2018 r1c1 (conf=0.696)}\\
    \includegraphics[width=\linewidth]{figures/mpm/mpm_srep2018_p2_img1_1650x950_r1c1_125x374_sam2.png}
    
    \scriptsize 绿框 = RF-DETR 检测，红框 = SAM2 mask bbox
    \column{0.48\textwidth}
    \centering
    \textbf{BMC Cancer 2023 Fig 1}\\
    \includegraphics[width=\linewidth]{figures/mpm/mpm_bmc_p4_img1_1353x1106_r2c1_285x837_sub1_285x187_sam2.png}
    
    \scriptsize RF-DETR 红框 + SAM2 绿框 高度一致
  \end{columns}
\end{frame}

\begin{frame}{MPM 数据获取计划：3 条路径}
  \begin{block}{路径 1：瑞金医工交叉（主路径，已有草稿）}
    \begin{itemize}
      \item PI：谢梦燕（瑞金肿瘤科） + 徐德昌（西浦 AI）
      \item 预期：20 例真实 MPM PDO + 临床病理信息
      \item 周期：2026.05-2027.04（1 年）
      \item 状态：申请书草稿已收，待修订提交
    \end{itemize}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{路径 2：4 封邮件请求论文作者}
    \scriptsize
    \begin{tabular}{lll}
      \toprule
      \textbf{作者} & \textbf{队列} & \textbf{优先级} \\
      \midrule
      Sarah Best (Sydney) & 44 PDO & P0 \\
      Li Chenggang (PLA) & 11 PDO & P0（已有合作） \\
      Marco Falasca (Nottingham) & 长期培养 & P1 \\
      Sarah Knox (UCSF) & 3 lines & P2 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{路径 3：公开 H\&E WSI 数据集（CC BY 4.0）}
    \begin{itemize}
      \item Nature Communications 2025: \textbf{3446 张 MPM WSI} (TCIA)
      \item TCGA-MESO: 87 例（基因组 + 临床）
      \item MesoGraph: 公开代码 + 数据
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{三本子联动逻辑}
  \begin{block}{递进式研究路径}
    \centering
    \large
    \textcolor{myblue}{\textbf{SURF 2026}} (代理验证)\\[0.3em]
    \scriptsize 18 学生 $\times$ 8 周\\[0.8em]
    $\Downarrow$\\[0.5em]
    \textcolor{mygreen}{\textbf{瑞金医工交叉}} (20 真实 MPM)\\[0.3em]
    \scriptsize 2026.05-2027.04\\[0.8em]
    $\Downarrow$\\[0.5em]
    \textcolor{myorange}{\textbf{国自然面上}} (100 例多中心)\\[0.3em]
    \scriptsize 2027-2030
    \normalsize
  \end{block}
  \vspace{-0.3em}
  \begin{block}{每阶段的产出}
    \begin{itemize}
      \item \textbf{SURF 2026}：算法可行性证据 + Streamlit 展示 + 18 学生毕设
      \item \textbf{瑞金}：20 例真实 MPM 数据 + 1 篇中文核心 + 1 项软著 + 代理验证论文
      \item \textbf{国自然}：100 例多中心 + 隐私攻击实测 + 临床验证 + 系统部署
    \end{itemize}
  \end{block}
  \vspace{-0.3em}
  \begin{alertblock}{核心论证}
    \begin{itemize}
      \item 代理数据（鼠肝+MultiOrg）验证算法 $\to$ 20 例真实 MPM 启动验证 $\to$ 100 例多中心扩展验证
      \item 今天的 3 个跨域实验为\textbf{代理验证策略}提供定量证据
    \end{itemize}
  \end{alertblock}
\end{frame}

% ========================================================================
\section{SAM2 自蒸馏验证（2026.07.16）}

\begin{frame}{SAM2 自蒸馏：方法原理}
  \begin{block}{核心思想}
    用 SAM2 zero-shot mask 作为\textbf{独立信号}，训练轻量分类头区分 TP/FP
    \begin{itemize}
      \item RF-DETR 跨域失效：conf 接近随机（median 0.014）
      \item SAM2 zero-shot 可用：IoU median 0.952
      \item 分类头学习 SAM2 信号 → 提升 TP/FP 判别
    \end{itemize}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{自蒸馏流程}
    \begin{enumerate}
      \item RF-DETR 检测 → 拿到 box + conf
      \item SAM2 box-prompted → 生成 mask
      \item 用 SAM2 IoU 作为伪标签（IoU≥0.50 = pos, <0.10 = neg）
      \item 训练分类头在 crop 特征上
      \item 评估 distilled score = conf × classifier\_prob
    \end{enumerate}
  \end{block}
  \vspace{-0.3em}
  \begin{alertblock}{关键设计：避免循环论证}
    不用 RF-DETR conf 当标签来源（否则 conf→label→学 conf = 自洽）
    用 SAM2 IoU 作\textbf{独立信号}
  \end{alertblock}
\end{frame}

\begin{frame}{三层证据链：Simulation → 真实跨域 → MPM 试点}
  \begin{block}{实验结果总览}
    \centering
    \small
    \begin{tabular}{lcccc}
      \toprule
      \textbf{实验} & \textbf{样本} & \textbf{Zero-shot} & \textbf{Distilled} & \textbf{提升} \\
      \midrule
      Simulation v2 (Intestinal) & 700 & 0.6616 & \textbf{0.9437} & \textcolor{mygreen}{\textbf{+28.22\%}} \\
      真实跨域 50+30 & 10578 & 0.7480 & \textbf{0.8857} & \textcolor{mygreen}{\textbf{+13.77\%}} \\
      真实跨域 200+100 & 40503 & 0.7679 & \textbf{0.8589} & \textcolor{mygreen}{\textbf{+9.10\%}} \\
  ...
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{Simulation v2 设计}
      \begin{itemize}
        \item Intestinal 500+200 张
        \item 模拟 RF-DETR conf 失效（0.05-0.50）
        \item 模拟 SAM2 IoU 信号（5\% 噪声）
        \item AUC 0.66 → 0.94
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{block}{真实跨域 200+100}
      \begin{itemize}
        \item 鼠肝 RF-DETR → Intestinal
        \item 28925 train + 11578 val
        \item TP conf 0.073 vs FP 0.038
        \item AUC 0.77 → 0.86 (+9.10\%)
      \end{itemize}
    \end{block}
  \end{columns}
\end{frame}

\begin{frame}{关键发现：Classifier Alone > Distilled}
  \begin{alertblock}{颠覆性发现}
    传统蒸馏公式 \texttt{distilled = conf × classifier\_prob} \textbf{不是最优}！
    当 RF-DETR conf 跨域失效时，conf 是噪声，乘 classifier 反而拉低性能。
  \end{alertblock}
  \vspace{-0.3em}
  \begin{block}{四实验对比：Classifier alone vs Distilled}
    \centering
    \small
    \begin{tabular}{lcccc}
      \toprule
      \textbf{实验} & \textbf{Zero-shot} & \textbf{Classifier} & \textbf{Distilled} & \textbf{最优} \\
      \midrule
      Simulation v2 & 0.6616 & \textcolor{mygreen}{\textbf{0.9792}} & 0.9437 & Classifier \\
      Real 200+100 & 0.7679 & 0.8202 & \textcolor{mygreen}{\textbf{0.8589}} & Distilled \\
      Real ResNet50 & 0.9156 & \textcolor{mygreen}{\textbf{0.9938}} & 0.9281 & Classifier \\
      MPM pilot & 0.9779 & N/A & 1.0000 & Distilled \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{-0.3em}
  \begin{block}{洞察：Classifier alone 在 2/3 实验中胜出}
    \begin{itemize}
      \item ResNet50 真特征 AUC=0.9938（接近完美）
      \item Distilled (0.9281) < Classifier (0.9938)：conf 乘法压低了高置信度样本
      \item 只有 zero-shot AUC 0.70-0.85 中间区域 distilled 才赢（互补性）
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{自适应蒸馏策略}
  \begin{block}{三段策略表}
    ...
    \begin{tabular}{lclc}
      \toprule
      \textbf{Zero-shot AUC} & \textbf{策略} & \textbf{公式} & \textbf{提升} \\
      \midrule
      $< 0.70$ & Classifier alone & \texttt{distilled = classifier\_prob} & \textcolor{mygreen}{\textbf{+31.76\%}} \\
      $0.70 - 0.85$ & Distilled & \texttt{conf × classifier\_prob} & \textcolor{mygreen}{\textbf{+9.10\%}} \\
      $> 0.85$ & Zero-shot & \texttt{distilled = conf} & maintain \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{-0.3em}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{A. 完全失效 (AUC<0.70)}
      \begin{itemize}
        \item RF-DETR conf 是噪声
        \item Classifier alone 最优
        \item 例：Simulation (+31.76\%)
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{block}{B. 部分失效 (0.70-0.85)}
      \begin{itemize}
        \item conf 和 classifier 互补
        \item Distilled 最优
        \item 例：Real 200+100 (+9.10\%)
      \end{itemize}
    \end{block}
  \end{columns}
  \vspace{-0.3em}
  \begin{alertblock}{论文创新点}
    传统蒸馏假设 conf 有效，但跨域场景这个假设不成立。
    自适应策略根据 zero-shot AUC 动态选择公式，
    比固定 conf×classifier 策略平均多 +5-15\%。
  \end{alertblock}
\end{frame}

\begin{frame}{ResNet50 真特征 vs 图像统计}
  \begin{block}{特征对比实验（真实跨域 200+84）}
    \centering
    \begin{tabular}{lccccc}
      \toprule
      \textbf{特征} & \textbf{维度} & \textbf{Zero-shot} & \textbf{Classifier} & \textbf{Distilled} & \textbf{最优} \\
      \midrule
      图像统计 & 30 & 0.9156 & 0.82 & 0.86 & Distilled \\
      \textbf{ResNet50} & \textbf{2048} & 0.9156 & \textcolor{mygreen}{\textbf{0.9938}} & 0.9281 & \textbf{Classifier} \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{-0.3em}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{ResNet50 特征优势}
      \begin{itemize}
        \item 2048 维 vs 30 维
        \item ImageNet pretrained 语义特征
        \item 单独分类 AUC=0.9938
        \item 接近完美 TP/FP 判别
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{block}{但 distilled 反而更弱}
      \begin{itemize}
        \item Classifier 0.9938
        \item Distilled 0.9281 (-6.6pp)
        \item conf 乘法压低高置信度
        \item 应该用 classifier alone
      \end{itemize}
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{总结}
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        ...
        \item[\checkmark] \textcolor{red}{\textbf{MPM 跨域验证：3 实验完整闭环}}
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{核心成果}
      \begin{itemize}
        ...
        \item \textcolor{red}{\textbf{MPM domain gap 0.272 (5x 同域)}}
        \item \textcolor{red}{\textbf{SAM2 IoU 0.952: 鸿沟在分类头}}
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0em}
  \begin{alertblock}{下一步}
    \begin{itemize}
      ...
      \item \textcolor{red}{\textbf{P0: 修订瑞金申请书 + 发 4 封数据请求邮件}}
      \item \textcolor{red}{\textbf{P1: SAM2 自蒸馏微调（10-20 例 MPM）}}
      \item 论文撰写 + 专利申请
    \end{itemize}
  \end{alertblock}
\end{frame}
