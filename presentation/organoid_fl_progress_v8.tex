...
\begin{frame}{未来工作：数据集扩展计划}
  \begin{block}{已使用数据集}
    \begin{tabular}{lcccc}
      \toprule
      \textbf{数据集} & \textbf{来源} & \textbf{张数} & \textbf{分辨率} & \textbf{状态} \\
      \midrule
      鼠肝 Batch 1 & 冬生实验室 & 10 & 2592$\times$1944 & \checkmark 完成 \\
      鼠肝 Batch 2 & 冬生实验室 & 10 & 4000$\times$3000 & \checkmark 完成 \\
      鼠肝 Batch 3 & 冬生实验室 & 20 & 4000$\times$3000 & \checkmark 完成 \\
      MultiOrg & NeurIPS 2024 & 411 & 6385$\times$5720 & \checkmark 完成 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{block}{计划扩展数据集}
    \begin{tabular}{lccc}
      \toprule
      \textbf{数据集} & \textbf{来源} & \textbf{特点} & \textbf{目标} \\
      \midrule
      肠类器官 & 合作实验室 & 不同器官类型 & 跨器官泛化 \\
      肺类器官 & Orga-Dete 论文 & 4008张, 已有 SOTA & 精度对标 81.4\% \\
      脑类器官 & 待收集 & 3D 结构复杂 & 形态学多样性 \\
      \midrule
      多实验室鼠肝 & 不同机构 & 不同显微镜/染色 & 预处理标准化 \\
      多倍率数据 & 4X/10X/20X & 分辨率归一化 & 尺度不变性 \\
      共聚焦/荧光 & 不同成像方式 & 多模态融合 & 跨模态 FL \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{核心方向}
    \begin{itemize}
      \item 图像预处理标准化模块（分辨率归一化 + CLAHE + 背景扣除 + 位深统一）
      \item 视觉原语框架：bbox $\to$ SAM2 mask $\to$ 形态学特征 $\to$ FL 聚合 primitive 分布
      \item 数据飞轮：检测 $\to$ 标注 $\to$ 训练 $\to$ 部署 $\to$ 反馈
    \end{itemize}
  \end{alertblock}
\end{frame}
