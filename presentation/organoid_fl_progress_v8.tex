...
\begin{frame}{总结}
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        \item[\checkmark] SOTA 全面调研 + MultiOrg 77.8\%（超论文 +3.9pp）
        \item[\checkmark] SAM2 三轮实验：微调=负优化；FP 抑制全面无效
        \item[\checkmark] Ensemble + Orga-Dete 迁移：均失败
        \item[\checkmark] MPCA Identity Init 铁律发现
        \item[\checkmark] 鼠肝 bbox FL 全部完成（E4-E11）
        \item[\checkmark] \textbf{B2 图片反序 Bug 修复}（mAP 0$\to$0.812）
        \item[\checkmark] \textbf{鼠肝 v2 14 实验全部完成}
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{鼠肝 v2 关键结果}
      \begin{itemize}
        \item B1 F1=\textbf{1.00}, B2 F1=\textbf{0.80}, B3=0.75
        \item B2 fewshot F1=\textbf{0.92} $>$ full 0.80
        \item F3 hard gate FL mAP50=\textbf{0.92}
        \item 传统 CV 完全无效（F1=0）
        \item Central 合并反而不如 B1 单独
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0em}
  \begin{alertblock}{下一步}
    bbox $\to$ SAM2 mask $\to$ 形态学特征 $\to$ FL 聚合 primitive 分布 $\to$ 论文撰写 + 专利申请
  \end{alertblock}
\end{frame}

\begin{frame}{PCB-Defect-FL：最早的联邦学习实验（2025）}
  \begin{block}{DeepPCB 数据集}
    \begin{itemize}
      \item 工业 PCB 缺陷检测：6 类（短路、断路、毛刺、缺口、鼠咬、杂铜）
      \item 1500 张（1200 train + 300 val），YOLO 格式
      \item 从检测数据集裁剪 bbox 区域 $\to$ DeepPCB\_cropped (8008/2005)
    \end{itemize}
  \end{block}
  \vspace{0em}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{Baseline (YOLOv12n, 集中式)}
      \begin{tabular}{lc}
        \toprule
        \textbf{指标} & \textbf{结果} \\
        \midrule
    ...
    \end{block}
    \column{0.48\textwidth}
    \begin{block}{FL 实验 (3-client non-IID)}
      \begin{itemize}
        \item EWA: $w_c = \exp(-H_c/\alpha)$ 熵加权
        \item TrustFL: $\kappa_c$ conformity 检测
        \item 3-client 下 entropy 权重 $\approx 0.33$（太均匀）
        \item conformity 检测更有效
      \end{itemize}
    \end{block}
  \end{columns}
  \vspace{0em}
  \begin{alertblock}{PCB $\to$ Organoid 的技术迁移}
    PCB-Defect-FL 的 EWA 熵加权 $\to$ organoid-fl 的 FL 监控框架；conformity 检测 $\to$ 幻觉防御；SAM2 分割 $\to$ 类器官形态分析。同一套核心算法跨领域复用。
  \end{alertblock}
\end{frame}
...
\begin{frame}{未来工作：数据集扩展计划}
  \begin{block}{已使用数据集}
    \begin{tabular}{lcccc}
      \toprule
      \textbf{数据集} & \textbf{来源} & \textbf{张数} & \textbf{类型} & \textbf{状态} \\
      \midrule
      DeepPCB & 公开 & 1500 & 工业缺陷 & \checkmark 完成 \\
      鼠肝 B1/B2/B3 & 冬生实验室 & 40 & 类器官检测 & \checkmark 完成 \\
      MultiOrg & NeurIPS 2024 & 411 & 类器官检测 & \checkmark 完成 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
