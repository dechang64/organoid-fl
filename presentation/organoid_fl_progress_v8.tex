...
\begin{frame}{鼠肝 v2：B2 修复后初测结果}
  \begin{block}{B2 RF-DETR 训练曲线（修复后）}
    \begin{tabular}{lcc}
      \toprule
      \textbf{Epoch} & \textbf{val mAP50} & \textbf{val mAP50-95} \\
      \midrule
      0 (after 1st step) & 0.555 & 0.419 \\
      1 & 0.794 & 0.593 \\
      8 (best) & \textbf{0.803} & \textbf{0.675} \\
      20 (final) & 0.812 & 0.645 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{修复前 $\to$ 修复后}
    \begin{tabular}{lcc}
      \toprule
      & \textbf{修复前} & \textbf{修复后} \\
      \midrule
      初始 loss & 15.4 & 正常 \\
      best val mAP50 & 0.011 & \textbf{0.812} \\
      best val mAP50-95 & 0.003 & \textbf{0.675} \\
      \bottomrule
    \end{tabular}
  \end{alertblock}
  \vspace{0em}
  \begin{block}{下一步}
    14 实验全部重跑中（B2 数据已修复，B1/B3 本来没问题）$\to$ 完整结果待更新
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        ...
        \item[\checkmark] \textbf{鼠肝 v2 14 实验矩阵设计 + 重跑}
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{下一步}
      \begin{itemize}
        \item[$\rightarrow$] 鼠肝 v2 14 实验全部重跑（B2 修复后）
        \item[$\rightarrow$] 完整 baseline + transfer + FL 结果分析
        \item[$\rightarrow$] bbox → SAM2 mask → 形态学特征
        \item[$\rightarrow$] FL 聚合 primitive 分布（非权重）
