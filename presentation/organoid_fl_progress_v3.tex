...
\begin{frame}{MultiOrg 检测精度 vs SOTA}
  \begin{block}{mAP@0.5 对比}
    \begin{tabular}{lcc}
      ...
      \midrule
      YOLOv12s patch 级 & 78.1\% & +10.0pp \\
      RF-DETR small SAHI 全图 & 77.13\% & +9.0pp \\
      \textbf{YOLOv12s SAHI 全图} & \textbf{68.8\%} & \textbf{+0.7pp} \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{结论}
    \begin{itemize}
      \item Patch 级超 SOTA \textbf{+10.0pp}，SAHI 全图超 SOTA \textbf{+0.7pp}
      \item RF-DETR NMS-free 结构在密集 organoid 上有结构性优势
      \item 距 80\% 门槛还差 2-3pp，瓶颈是数据量（600张）
    \end{itemize}
  \end{alertblock}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        ...
        \item[\checkmark] MultiOrg patch 级超 SOTA（+10pp）
        \item[\checkmark] MultiOrg SAHI 全图超 SOTA（+0.7pp）
        \item[\checkmark] SAM2 自蒸馏微调（val IoU 0.87）
        \item[\checkmark] 代码系统审计（7 bug 修复）
    ...
    \begin{block}{进行中 / 待完成}
      \begin{itemize}
        \item[$\circ$] SAM2 对照实验（zero-shot vs finetuned）
