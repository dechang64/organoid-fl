...
\begin{frame}{跨域泛化：MultiOrg $\to$ 鼠肝（进行中）}
  \begin{block}{动机：传统方法跨域失败}
    \begin{itemize}
      \item B1$\to$B2 zeroshot: F1=0.22（大目标$\to$小目标崩溃）
      \item B1$\to$B3 zeroshot: F1=0.00（完全失效）
      \item SAM2 fewshot: B2 F1=0.92, B3=0.67（需目标域标注）
    \end{itemize}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{Slot 跨域假设}
    \begin{itemize}
      \item DINOv2 冻结 $\to$ 不依赖目标域微调
      \item Slot 学到的是 ``organoid 长什么样''（object-centric 原语）
      \item 不是 ``MultiOrg 的 bbox 分布''
      \item $\to$ \textbf{直接迁移到鼠肝应该有效}
    \end{itemize}
  \end{alertblock}
  \vspace{0em}
  \begin{block}{实验矩阵（G2 = 核心）}
    \begin{itemize}
      \item G2: MultiOrg best.pt $\to$ 鼠肝 B1/B2/B3（零标注跨域）
      \item 判定标准：Slot AUC $>$ 0.55 $\to$ 跨域有效
      \item 如果成功 $\to$ ``通用 organoid 原语''论文叙事
    \end{itemize}
  \end{block}
\end{frame}
