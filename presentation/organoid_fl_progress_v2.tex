...
\setbeamertemplate{block begin}{\vskip0.5em\begin{beamercolorbox}[ht=2.5ex,dp=0.8ex,leftskip=0.5em,rightskip=0.5em]{block title}\usebeamerfont*{block title}\insertblocktitle\end{beamercolorbox}\vskip1.0ex\begin{beamercolorbox}[dp=0.8ex,leftskip=0.5em,rightskip=0.5em]{block body}\usebeamerfont{block body}}
\setbeamertemplate{block end}{\end{beamercolorbox}\vskip0.5em}
\setbeamertemplate{block alerted begin}{\vskip0.5em\begin{beamercolorbox}[ht=2.5ex,dp=0.8ex,leftskip=0.5em,rightskip=0.5em]{block title alerted}\usebeamerfont*{block title}\insertblocktitle\end{beamercolorbox}\vskip1.0ex\begin{beamercolorbox}[dp=0.8ex,leftskip=0.5em,rightskip=0.5em]{block body alerted}\usebeamerfont{block body}}
\setbeamertemplate{block alerted end}{\end{beamercolorbox}\vskip0.5em}
...
\begin{frame}{类器官检测 SOTA 全景}
  \begin{block}{数据集与 SOTA}
    \begin{tabular}{p{3.2cm}p{3.5cm}cc}
      \toprule
      数据集 & SOTA 方法 & mAP@0.5 & 年份 \\
      \midrule
      Intestinal & Deliod (YOLOv8s) & 87.5\% & 2025 \\
      MultiOrg & SSD & 68.1\% & 2024 \\
      Colorectal & YOLOv4 & 36.9\% & 2023 \\
      Gastric & YOLOv5 & 79\% acc & 2023 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{block}{关键发现}
    \begin{itemize}
      \item \textbf{小目标检测是核心瓶颈}：改进都围绕小目标（+11.6pp vs 分类+0.7pp）
      \item \textbf{数据增强 > 架构改进}：Orga-Dete 增强 +3.5pp，BiFPN+MPCA +1.5pp
      \item \textbf{NMS 误删密集目标}：传统 NMS 对密集 organoid 有结构性缺陷
...
\begin{frame}{SOTA 检测器对比（2025--2026）}
  \begin{block}{通用检测 SOTA}
    \begin{tabular}{llcl}
      ...
      模型 & 类型 & COCO mAP & 优势 \\
      \midrule
      YOLOv12 & CNN+Attn & 55.9\% & 速度最优 \\
      RF-DETR & Transformer & \textbf{60+}\% & NMS-free \\
      D-FINE & Transformer & 59.5\% & 定位精度 \\
      RT-DETRv3 & Transformer & 55.3\% & 实时DETR \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{block}{Organoid 检测 SOTA}
    \begin{tabular}{lll}
      ...
      工作 & 模型 & mAP@0.5 \\
      \midrule
      Bukas et al. (NeurIPS 2024) & SSD & 68.1\% \\
      Yang et al. (Biomimetics 2025) & RTMDet+IDP-Head & +5pp \\
      Orga-Dete (Appl. Sci. 2025) & YOLOv8-Lite & TIDE 分析 \\
      MEF-YOLO (Frontiers 2025) & YOLOv10n & fine-grained \\
      \textbf{Ours} & YOLOv12s+SAHI & \textbf{68.8\%} \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{RF-DETR 的结构性优势}
    NMS-free 架构在密集 organoid 场景无 NMS 误删问题
  \end{alertblock}
\end{frame}
...
\begin{frame}{SAHI 优化的理论基础}
  ...
  \begin{block}{优化策略与文献依据}
    \begin{itemize}
      \item \textbf{Overlap}（YOLO-Patch-Based-Inference, 2025）：15\%--40\% 最优；本实验 0.3
      \item \textbf{Soft-NMS}（Bodla et al., ICCV 2017）：高斯衰减，保留更多 TP
      \item \textbf{WBF vs NMS}（Solovyev et al.）：Soft-NMS 在 AP 上最优
      \item \textbf{论文协议}（NeurIPS 2024）：512+2048 双尺度 + NMS 0.5
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{SAHI 全图推理优化}
  \begin{block}{问题：Patch 级 vs 全图推理差距}
    Patch 级 78.1\% → SAHI 全图 64.8\%（$-$13.3pp），瓶颈是 \alert{FP 暴增}
  \end{block}
  \vspace{0em}
  \begin{block}{参数优化实验}
    \begin{tabular}{lcccc}
      \toprule
      配置 & mAP@0.5 & Prec & Rec & FP \\
      \midrule
      WBF, o=0.5 & 64.8\% & 48.0\% & 83.0\% & 4463 \\
      NMS, o=0.5 & 62.4\% & 42.8\% & 86.3\% & 5720 \\
      WBF, o=0.3 & 66.1\% & 52.8\% & 81.3\% & 3602 \\
      \textbf{Soft-NMS, o=0.3} & \textbf{68.8\%} & 27.0\% & 85.5\% & 11466 \\
      \midrule
      + score filter 0.3 & 66.5\% & \textbf{52.1\%} & 81.1\% & 3690 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{超 SOTA}
    论文报告 mAP@0.5 = \textbf{68.8\%}（超 SSD \textbf{+0.7pp}）；部署版 Precision 52.1\%
  \end{alertblock}
\end{frame}
