...
\begin{frame}{SAHI 优化的理论基础}
  ...
  \begin{block}{优化策略与文献依据}
    \begin{itemize}
      ...
      \item \textbf{负样本训练}（Bulten et al., PMC 2021）：加入空白 patch，FP 62.5\%$\rightarrow$25\%
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{数据预处理：从错误到正确}
  ...
  \begin{block}{标注者选择 bug（本次新发现）}
    \begin{itemize}
      \item Test set 有 3 套标注（t0/t1\_A/t1\_B），代码合并后 \alert{GT 重复 3 倍}
      \item 同一 organoid 被标 3 次 → 1 TP + 2 FN → recall 暴跌
      \item 修复：非 multi-rater 模式只用 primary 标注者
      \item 影响：mAP@0.5 从 \textbf{40.6\% → 66.9\%}（+26.3pp）
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{从检测优化到范式转换}
  \begin{block}{当前范式的根本局限}
    \begin{itemize}
      \item 端到端"像素$\rightarrow$bbox"把概念固化在权重中
      \item 换领域/标注者/中心 $\rightarrow$ 权重失效，需重训
      \item SAHI/NMS/Soft-NMS 都是后处理治标，不治本
    \end{itemize}
  \end{block}
  \vspace{0.3em}
  \begin{alertblock}{启发：Thinking with Visual Primitives (DeepSeek-AI, 2025)}
    将视觉原语（点、bbox）作为推理的最小单元，不依赖自然语言指代。
    \begin{itemize}
      \item 解决 Reference Gap：语言描述空间位置有歧义
      \item 保留指代不确定性：不强迫硬决策
    \end{itemize}
  \end{alertblock}
  \vspace{0.3em}
  \begin{block}{映射到我们的问题}
    \begin{itemize}
      \item Soft-NMS 赢 6pp 正是因为保留"指代不确定性"
...
\begin{frame}{三层通用视觉框架设想}
  \begin{block}{Layer 1: Perception（感知层）}
    \begin{itemize}
      \item SAM2/SegAny 输出无类别 visual primitives（mask + 位置 + 形态）
      \item 不学习"这是什么"，只学习"这里有什么"
      \item \textbf{跨域通用}：肺器官、肠器官、PCB 缺陷，同一套感知
    \end{itemize}
  \end{block}
  \vspace{0.3em}
  \begin{block}{Layer 2: Reasoning（推理层）}
    \begin{itemize}
      \item VLM/LLM 看着 visual primitives + 全图上下文做推理
      \item 泛化来源：语义理解，不是像素匹配
      \item 推理过程可解释（Chain-of-Thought）
    \end{itemize}
  \end{block}
  \vspace{0.3em}
  \begin{block}{Layer 3: Collaboration（协作层）}
    \begin{itemize}
      \item FL 不再聚合权重，聚合 visual primitive 分布
      \item 各中心上传"形态统计"，不上传像素/权重
      \item \textbf{真正实现"数据不动，知识动"}
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{已有探索与技术栈}
  \begin{block}{EWA-Fed: 视觉原语编解码（已实现）}
    \begin{itemize}
      \item \texttt{primitives.py}: BOX/POINT/PATH 原语编解码，Shannon 熵
      \item \texttt{aggregator.py}: 熵加权聚合，per-class 统计
      \item \texttt{conformity.py}: 跨 client 从众检测
      \item 当前仅用于监控，未用于推理替代检测
    \end{itemize}
  \end{block}
  \vspace{0.3em}
  \begin{block}{技术栈映射}
...
\begin{frame}{总结}
  \begin{block}{已完成}
    \begin{itemize}
      \item[\checkmark] SOTA 全面调研（MultiOrg + Intestinal + 专利）
      \item[\checkmark] 数据预处理管线（tiling + 多标注者支持）
      \item[\checkmark] 标注 bug 修复（GT 3x 重复 → mAP +26pp）
      \item[\checkmark] SAHI 全图推理超 SOTA（68.8\% vs 68.1\%，Soft-NMS + overlap=0.3）
      \item[\checkmark] 评估方法论（标注者影响量化：12pp 差异）
      \item[\checkmark] Patch 级精度超 SOTA（78.1\% vs 68.1\%，+10.0pp）
      \item[\checkmark] EWA Effective Interval 理论 + 实验验证
    \end{itemize}
  \end{block}
  \vspace{0.3em}
  \begin{block}{进行中 / 待完成}
    \begin{itemize}
      \item[$\circ$] RF-DETR 对比实验（NMS-free Transformer）
      \item[$\circ$] 负样本训练（空白 patch 减少 FP，文献预期 FP 62.5\%→25\%）
