...
\begin{frame}{SAM2 对照实验结果}
  \begin{block}{全量 55 张测试，mask IoU bug 修复后}
    \begin{tabular}{lcccc}
      ...
      \midrule
      Zero-shot SAM2 & 待跑 & 待跑 & — & — \\
      Finetuned v2 (4点GT) & 待跑 & 待跑 & — & — \\
      \textbf{Finetuned pseudo} & \textbf{77.13\%} & \textbf{75.89\%} & \textbf{43.9\%} & 11553 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{block}{形态学过滤无效（复现鼠肝结论）}
    \begin{tabular}{lcccc}
      \toprule
      Filter & mAP50 & Prec & Rec & F1 \\
      \midrule
      baseline & 77.1\% & 28.6\% & 93.6\% & 43.9\% \\
      conf$\geq$0.4 & 75.1\% & 43.8\% & 88.2\% & 58.5\% \\
      conf$\geq$0.5 & 70.5\% & 60.2\% & 79.0\% & 68.4\% \\
      circ/solid/ar & \multicolumn{4}{c}{砍 FP $<$ 2\%，无效} \\
      \bottomrule
    \end{tabular}
  \end{block}
  \vspace{0em}
  \begin{alertblock}{结论}
    形态学过滤对 MultiOrg 无效。conf 是唯一有效维度，但阈值高了砍 recall。瓶颈在检测器，不在后处理。
  \end{alertblock}
\end{frame}
...
\begin{frame}{突破 80\% 的方向：Orga-Dete 迁移}
  \begin{block}{Orga-Dete 三模块（Huang et al., Applied Sciences 2025）}
    \begin{tabular}{lll}
      \toprule
      模块 & 作用 & 消融贡献 \\
      \midrule
      BiFPN & 双向特征金字塔 & small +3.7\% \\
      MPCA & 多路径坐标注意力 & small +4.0\% \\
      EMASlideLoss & 动态阈值 loss & 零成本提升 \\
      \bottomrule
    \end{tabular}
  \end{block}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{MultiOrg 匹配度}
      \begin{itemize}
        \item[\checkmark] 小目标多 → BiFPN/MPCA 对症
        \item[\checkmark] 类别不平衡 → EMASlideLoss
        \item[\checkmark] 密集场景 → BiFPN 抑制噪声
        \item[\checkmark] 3 个数据集验证跨域泛化
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{预期收益}
      \begin{itemize}
        \item YOLOv11n baseline $\sim$77.9\%
        \item +三模块 $\sim$+3-4pp
        \item 预期：\textbf{80-82\%}
        \item 实现：3-5 天
        \item 代码未开源，需自实现
  ...
  \begin{columns}[T]
    ...
    \begin{block}{已完成}
      \begin{itemize}
        ...
        \item[\checkmark] SAM2 自蒸馏微调（val IoU 0.87）
        \item[\checkmark] Ensemble 实验（WBF/Union/Intersection）
        \item[\checkmark] Ensemble 失败分析（YOLO 严格劣势）
    ...
    \begin{block}{下一步}
      \begin{itemize}
        \item[$\rightarrow$] \textbf{Orga-Dete 迁移：EMASlideLoss（零成本）}
        \item[$\rightarrow$] BiFPN + MPCA 模块实现
