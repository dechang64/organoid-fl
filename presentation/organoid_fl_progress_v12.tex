...
\begin{frame}{跨域泛化：MultiOrg $\to$ 鼠肝 \& Intestinal（结果）}
  ...
  \end{block}
  \vspace{0em}
  \begin{alertblock}{结果：Slot 跨域失效}
    \footnotesize
    \begin{tabular}{lcccc}
      \toprule
      \textbf{数据集} & \textbf{Crops} & \textbf{Slot AUC} & \textbf{Conf AUC} & \textbf{融合提升} \\
      \midrule
      鼠肝 B1 (2592$\times$1944) & 26 & \textcolor{red}{0.29}（反预测） & 0.91 & 无 \\
      鼠肝 B2 (4000$\times$3000) & 48 & \textcolor{red}{0.51}（随机） & 0.98 & 无 \\
      鼠肝 B3 (4000$\times$3000) & 60 & \textcolor{red}{0.54}（随机） & 0.92 & 无 \\
      Intestinal (84 val) & 2744 & \textcolor{red}{0.67} & 0.92 & +0.0005 \\
      \bottomrule
    \end{tabular}
  \end{alertblock}
  \vspace{0em}
  \begin{block}{结论}
    \begin{itemize}
      \item SupCon 学到的 TP/FP 区分是\textbf{域特定}的，不构成通用 organoid 原语
      \item RF-DETR/YOLO confidence 跨域稳健（不依赖域知识）
      \item 方向：需更大数据量 + 跨域训练（多数据集联合训练）
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{总结}
  \begin{columns}[T]
    ...
    \begin{alertblock}{核心成果}
      \begin{itemize}
        ...
        \item object-centric 分解有效（同域）
        \item SupCon $\beta=0.1$ 最优
        \item \textcolor{red}{跨域失效}：slot 域特定
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0em}
  \begin{alertblock}{下一步}
    \begin{itemize}
      \item 跨域泛化：多数据集联合训练（MultiOrg + 鼠肝 + Intestinal）
      \item 更大数据量：100 crops $\to$ 16K crops（CTM 已验证数据量是关键）
      \item 论文撰写 + 专利申请
    \end{itemize}
  \end{alertblock}
\end{frame}
