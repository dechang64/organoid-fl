...
def process_image_multi_rater(img_dir, output_img_dir, output_lbl_dirs,
    ...
    # 选择 primary 标注者（用于决定哪些 patch 有内容）
    # 优先级: t0 > annotator_a > t1_a > annotator_b > t1_b > other
    PRIORITY = ['t0', 'annotator_a', 't1_a', 'annotator_b', 't1_b', 'other']
    primary_key = next(
        (k for k in PRIORITY if k in all_annotations),
        list(all_annotations.keys())[0]
    )
...
def process_split(src_dir, dst_dir, split, tile_size, stride,
    ...
    if multi_rater:
        ...
        # test set（非 multi-rater）：为每个标注者生成单独的 labels 目录
        # 这样可以用 t0/t1_a/t1_b 分别评估
        # 同时复制 t0 到 labels/ 作为默认
        ann_types = set()
        for class_name in os.listdir(split_dir):
            class_dir = os.path.join(split_dir, class_name)
            if not os.path.isdir(class_dir):
        ...
        # train set（非 multi-rater）：合并到 labels/
        # train 每图只有1个标注者(A或B)，合并不会重复
...
def create_data_yaml(dst_dir):
    ...
    yaml_content = f"""# MultiOrg YOLO Dataset v3 (single-class, 640px)
# Generated by multiorg_tiling_v3.py
# val labels: test/labels/ (default = t0 copy)
# To evaluate with t1_a: replace test/labels/ with test/labels_t1_a/
# To evaluate with t1_b: replace test/labels/ with test/labels_t1_b/
path: {dst_fwd}
train: train/images
val: test/images
...
def setup_default_val_labels(dst_dir):
    """Copy labels_t0 → labels as default val labels.
    Creates switch_val.bat for easy switching between t0/t1_a/t1_b.
    """
    import shutil
    test_dir = os.path.join(dst_dir, 'test')
    labels_t0 = os.path.join(test_dir, 'labels_t0')
    labels_default = os.path.join(test_dir, 'labels')

    if os.path.isdir(labels_t0):
        if os.path.isdir(labels_default):
            shutil.rmtree(labels_default)
        shutil.copytree(labels_t0, labels_default)
        print(f"  Copied labels_t0 → labels (default val)")
    ...
    with open(switch_script, 'w') as f:
        ...
        f.write(f'set TEST_DIR={test_dir}\n')
        f.write('rmdir /s /q "%TEST_DIR%\\labels"\n')
        f.write('xcopy /e /i /q "%TEST_DIR%\\labels_%ANNOTATOR%" "%TEST_DIR%\\labels"\n')
        f.write('echo Switched val labels to %ANNOTATOR%\n')
    print(f"  Created switch_val.bat (usage: switch_val.bat t0|t1_a|t1_b)")
...
def main():
    ...
    setup_default_val_labels(args.dst)
