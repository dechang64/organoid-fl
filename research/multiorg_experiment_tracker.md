...
- L1: Train on MultiOrg + Intestinal → Test on 鼠肝 B1/B2/B3
- L2: Train on MultiOrg + 鼠肝 → Test on Intestinal
- L3: Train on 鼠肝 + Intestinal → Test on MultiOrg

| 数据集 | 单域 AUC | 同域（含泄露） | **LOO（真跨域）** | Conf AUC | 泄露量 |
|--------|----------|---------------|------------------|----------|--------|
| 鼠肝 B1 | 0.29 | 0.74 | **0.49** | 0.91 | 0.25 |
| 鼠肝 B2 | 0.51 | 0.70 | **0.56** | 0.98 | 0.14 |
| 鼠肝 B3 | 0.54 | 0.85 | **0.82** | 0.92 | 0.03 |
| Intestinal | 0.67 | 0.74 | **0.58** | 0.92 | 0.16 |

- **同域结果有数据泄露**：平均泄露 0.14pp（同域 0.74 → LOO 0.61 平均）
- **B3 最接近 conf**：LOO 0.82 vs conf 0.92（差 10pp），说明 B3 和训练集特征重叠最多
- **Intestinal LOO 比单域还差**（0.58 < 0.67）：MultiOrg+鼠肝训练引入了干扰
- **B1/B2 LOO 接近随机**（0.49/0.56）：联合训练未学到通用原语
- **结论**：多数据集联合训练的"提升"大部分来自数据泄露，DINOv2 特征的跨域泛化仍然失败
