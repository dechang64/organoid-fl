...
### 9.5 多数据集联合训练（2026-07-14）

**训练数据**: MultiOrg 100 + 鼠肝 134 + Intestinal 2744 = ~3000 crops 联合训练

**注意**：以下结果中三个数据集都参与了训练，评估时用的是训练集包含的数据集——属于**同域测试**（非跨域），不能称为跨域泛化提升。真正的跨域评估需要留一法（见 §9.6）。

| 数据集 | 单域 Slot AUC | 联合训练后 AUC | Conf AUC | 提升 | 最优融合 Δ |
|--------|---------------|---------------|----------|------|-----------|
| 鼠肝 B1 | 0.29（反预测） | **0.74** | 0.91 | +0.45 | +0.0042 (soft) |
| 鼠肝 B2 | 0.51（随机） | **0.70** | 0.98 | +0.19 | +0.0020 (soft) |
| 鼠肝 B3 | 0.54（随机） | **0.85** | 0.92 | +0.31 | +0.0125 (soft) |
| Intestinal | 0.67 | **0.74** | 0.92 | +0.07 | +0.0012 (geo) |

- **联合训练在同域测试上提升显著**：B1 从 0.29→0.74，B3 从 0.54→0.85
- **但仍未超过 conf baseline**：所有场景 slot 仍低于 conf
- **⚠️ 以上不是跨域结果**——三个数据集都在训练集中
- **下一步**：需要留一法验证真正的跨域泛化（见 §9.6）

### 9.6 留一法真正跨域评估（2026-07-14）

**方法**：轮流留一个数据集做测试集，用另外两个训练。

| 数据集 | DINOv2 单域 | DINOv2 LOO | CLIP zero-shot | CLIP+SupCon LOO | CoOp LOO | Conf |
|--------|------------|-----------|---------------|-----------------|----------|------|
| MultiOrg | 0.79 | — | 0.73 | 0.95 | 0.62 | 0.87 |
| 鼠肝 B1 | 0.29 | 0.49 | **0.86** | 0.50 | 0.67 | 0.91 |
| 鼠肝 B2 | 0.51 | 0.56 | **0.66** | 0.57 | 0.58 | 0.98 |
| 鼠肝 B3 | 0.54 | 0.82 | **0.69** | 0.45 | 0.65 | 0.92 |
| Intestinal | 0.67 | 0.58 | **0.69** | 0.50 | 0.69 | 0.92 |
| **平均** | 0.50 | 0.58 | **0.733** | 0.578 | 0.642 | 0.940 |

### 跨域方法排名
1. **Conf baseline: 0.940**（冠军，不可逾越）
2. **CLIP zero-shot: 0.733**（最强跨域 slot，无训练）
3. CoOp LOO: 0.642（训练 prompt，破坏泛化）
4. DINOv2 LOO: 0.583（训练 slot on DINOv2）
5. CLIP+SupCon LOO: 0.578（训练 slot on CLIP）

### 核心结论

- **所有监督训练都破坏 CLIP 的 zero-shot 跨域能力**
  - SupCon 训练 slot → B1 从 0.86 降到 0.50（-0.36）
  - CoOp 训练 prompt → B1 从 0.86 降到 0.67（-0.19，破坏更少但仍不如 zero-shot）
- **CLIP zero-shot 是最强跨域方法**（无训练，平均 0.733）
- **MultiOrg 同域 CLIP+SupCon 0.95 > conf 0.87**（+7.8pp）——同域有效但跨域失效
- **根因**：监督信号让模型过拟合到源域，破坏预训练泛化（和 NLP 中微调破坏 BERT zero-shot 一致）
- **方向**：不训练参数，改用 prompt 工程 / 多 prompt 集成 / VLM 推理 / 3D 时序
