...
| 数据集 | 单域 Slot AUC | 联合 Slot AUC | Conf AUC | 提升 | 最优融合 Δ |
|--------|---------------|---------------|----------|------|-----------|
...
- **联合训练大幅提升跨域泛化**：B1 从反预测→0.74，B3 从随机→0.85
- **B3 接近 conf baseline**（差 7pp），soft_penalize 首次有实质正 delta
- **但仍未超过 conf**：单帧 DINOv2 特征天花板
- **下一步**：CLIP 语义对齐 / 3D 时序 / VLM 推理操作（见文献调研报告）
